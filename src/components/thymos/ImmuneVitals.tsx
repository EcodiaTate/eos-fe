...
import { api, type ThymosHealthResponse } from "@/lib/api-client";
...
export function ImmuneVitals() {
  ...
  const healthScore = Math.min(
    ...
    Math.max(
      0,
      100 -
        (data.active_incidents * 2 +
          (data.repairs_failed / Math.max(1, data.repairs_attempted)) * 50) /
          2
    )
  );
        ...
        <Card className="bg-slate-700/50 border-slate-600">
          ...
          <CardContent>
            <div className="text-4xl font-bold text-green-400">
              {data.repairs_attempted > 0
                ? Math.round(
                    (data.repairs_succeeded / data.repairs_attempted) * 100
                  )
                : 0}
              %
            </div>
            ...
            {data.budget && (
              <>
                <div className="text-2xl font-bold text-cyan-400">
                  {(data.budget as any)?.remaining_in_window ?? "—"}
                </div>
                <div className="text-xs text-slate-400">
                  repairs remaining in window
                </div>
              </>
            )}
          </CardContent>
